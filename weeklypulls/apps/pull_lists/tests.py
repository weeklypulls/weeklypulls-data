...
class MUAlertsTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.pull_list = PullList.objects.create(
            title=f"{__name__} test list", owner=cls.test_user, mu_enabled=True
        )
        MUPull.objects.create(series_id=cls.series_1, pull_list=cls.pull_list)
        MUPull.objects.create(series_id=cls.series_2, pull_list=cls.pull_list)
    ...
    def test_weekly_view_correct(self):
        ...
        alert3 = MUPullAlert.create_for_issue(
            issue_fail, self.series_1, six_months_ago.replace(day=+14)
        )

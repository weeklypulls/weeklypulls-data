...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f"DRY RUN - No API calls will be made")
            )

        self.stdout.write(
            f"Starting ComicVine data fetch with limit of {limit} API requests..."
        )
        ...
        for volume_id in missing_volumes:
            ...
            if dry_run:
                ...
                try:
                    ...
                    if volume:
                        self.stdout.write(
                            f"✅ Fetched volume {volume_id}: {volume.name}"
                        )
                    requests_made += 1
        ...
        self.stdout.write(
            f"Found {len(expired_volumes)} expired volumes (smart refresh: recent series weekly, older series monthly)"
        )
        ...
        for volume in expired_volumes:
            ...
            if dry_run:
                age_info = (
                    f" (started {volume.start_year})" if volume.start_year else ""
                )
                self.stdout.write(
                    f"Would refresh volume {volume.cv_id}: {volume.name}{age_info}"
                )
                requests_made += 1  # Count the request that would be made
            else:
                try:
                    ...
                    if refreshed:
                        age_info = (
                            f" (started {volume.start_year})"
                            if volume.start_year
                            else ""
                        )
                        self.stdout.write(
                            f"🔄 Refreshed volume {volume.cv_id}: {volume.name}{age_info}"
                        )
                    requests_made += 1
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(
                            f"❌ Failed to refresh volume {volume.cv_id}: {e}"
                        )
                    )
                    requests_made += 1
        ...
        volumes_needing_issues = self._get_volumes_needing_issues(
            max_volumes=remaining_requests
        )
        self.stdout.write(
            f"Found {len(volumes_needing_issues)} volumes needing issue data (prioritizing recent series, limited by {remaining_requests} remaining requests)"
        )
        ...
        for volume in volumes_needing_issues:
            ...
            if dry_run:
                ...
                self.stdout.write(
                    f"Would fetch issues for volume {volume.cv_id}: {volume.name}{age_info}"
                )
                requests_made += 1  # Count the request that would be made
            else:
                try:
                    ...
                    self.stdout.write(
                        f"📚 Fetched {len(issues)} issues for {volume.name}{age_info}"
                    )
                    requests_made += 1
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(
                            f"❌ Failed to fetch issues for volume {volume.cv_id}: {e}"
                        )
                    )
                    requests_made += 1
        ...
        if dry_run:
            self.stdout.write(
                self.style.SUCCESS(
                    f"DRY RUN complete. Would have made ~{requests_made} API requests"
                )
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    f"ComicVine fetch complete. Made {requests_made} API requests (limit: {limit})"
                )
            )
    ...
    def _get_expired_volumes(self, force_failed=False):
        ...
        recent_series = Q(start_year__gte=current_year - 3) & Q(
            last_updated__lt=recent_cutoff
        )

        # Priority 2: Older series (4-10 years old) - refresh if older than 2 weeks
        older_cutoff = now - timedelta(days=14)
        older_series = Q(
            start_year__gte=current_year - 10, start_year__lt=current_year - 3
        ) & Q(last_updated__lt=older_cutoff)

        # Priority 3: Very old series (10+ years old) - refresh if older than 1 month
        very_old_cutoff = now - timedelta(days=30)
        very_old_series = Q(start_year__lt=current_year - 10) & Q(
            last_updated__lt=very_old_cutoff
        )

        # Get recent series first (most likely to have new issues)
        recent_volumes = list(
            ComicVineVolume.objects.filter(query & recent_series).order_by(
                "-start_year", "last_updated"
            )[:10]
        )

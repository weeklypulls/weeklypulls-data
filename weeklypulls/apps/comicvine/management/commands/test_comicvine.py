...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--volume-id", type=int, help="Test with a specific ComicVine volume ID"
        )
        parser.add_argument(
            "--rate-limit-status",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if options["volume_id"]:
            ...
            if volume:
                ...
                self.stdout.write(
                    self.style.ERROR(f"✗ Failed to fetch volume {volume_id}")
                )
            return
        ...
        if options["cache_all_volumes"] or options["force_refresh_all"]:
            ...
            self.stdout.write(
                f"Starting {action} of all volumes from existing pulls..."
            )
            ...
            if options["force_refresh_all"]:
                ...
                already_cached = set()
                self.stdout.write(
                    f"Found {len(all_volumes)} unique volumes to force refresh"
                )
            else:
                ...
                already_cached = set(
                    ComicVineVolume.objects.filter(
                        cv_id__in=all_volumes, api_fetch_failed=False
                    )
                    .exclude(cache_expires__lt=timezone.now())  # Exclude expired cache
                ...
                if not volumes_to_fetch:
                    self.stdout.write(
                        self.style.SUCCESS("All volumes are already cached and fresh!")
                    )
                    return
